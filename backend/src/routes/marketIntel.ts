...
export async function marketIntelRoutes(app: FastifyInstance) {
  ...
  /** 시장 국가별 뉴스 요약. 관심 분야(categories=b2b,b2c,both) 선택 시 서버에서 필터 */
  app.get<{ Querystring: { country?: string; lang?: string; categories?: string } }>("/news-summary", async (req) => {
    const country = req.query?.country ?? "";
    const lang = (req.query?.lang ?? "ko") as "ko" | "en" | "es";
    const validLang = ["ko", "en", "es"].includes(lang) ? lang : "ko";
    const rawCategories = req.query?.categories;
    const categories: ("b2b" | "b2c" | "both")[] = rawCategories
      ? (rawCategories.split(",").map((c) => c.trim().toLowerCase()).filter((c): c is "b2b" | "b2c" | "both" => ["b2b", "b2c", "both"].includes(c)))
      : [];
    const items = await getMarketNewsSummaryAsync(country, validLang, categories.length ? { categories } : undefined);
  ...
  /** 시장 리포트 생성: 데이터 수집 후 선택 포맷(PDF/DOCX/Excel/JSON)으로 파일 생성 */
  app.post<{ Body: { orgId?: string; country?: string } }>("/generate-report", async (req) => {
    const orgId = (req.body?.orgId as string) ?? "default";
    const country = (req.body?.country as string) ?? "ALL";
    const job = await generateMarketReport(orgId, country);
    return job;
  });

  /** 생성된 리포트 파일 다운로드 */
  app.get<{ Params: { jobId: string } }>("/reports/:jobId/download", async (req, reply) => {
    const meta = getReportFileMeta(req.params.jobId);
    if (!meta) return reply.status(404).send({ error: "Report not found or expired" });
    try {
  ...
  /** 의뢰 진행: 세분화 의뢰 저장 후 시장조사(세분화 결과) + 해당 국가 뉴스 취합 반환 */
  const validResearchTypes = ["import", "export", "distribution", "consumption", "manufacturing"] as const;
  app.post<{
    Body: { orgId?: string; country?: string; lang?: string; country_code?: string; item?: string; hs_code?: string; research_types?: string[] };
  }>("/run-research", async (req, reply) => {
    try {
      const body = req.body ?? {};
      const orgId = (body.orgId as string) ?? "default";
      const country = (body.country as string) ?? "ALL";
      const lang = (body.lang as string) ?? "ko";
      const validLang = ["ko", "en", "es"].includes(lang) ? lang : "ko";
      const country_code = ((body.country_code as string) ?? "").trim();
      let rawTypes: string[] = [];
      if (Array.isArray(body.research_types)) rawTypes = body.research_types as string[];
      else if (typeof body.research_types === "string") rawTypes = (body.research_types as string).split(",").map((s) => s.trim()).filter(Boolean);
      const types = rawTypes.filter((t): t is (typeof validResearchTypes)[number] => validResearchTypes.includes(t as (typeof validResearchTypes)[number]));
      if (!country_code) {
        return reply.status(400).send({ error: "country_code is required for run-research" });
      }
      if (types.length === 0) {
        return reply.status(400).send({ error: "At least one research_type is required" });
      }
      const request = {
        country_code,
        item: (body.item as string) ?? "",
        hs_code: (body.hs_code as string) ?? "",
        research_types: types,
      };
      const result = await runMarketResearch(orgId, country, request, validLang as "ko" | "en" | "es");
      return result;
    } catch (err) {
      const message = err instanceof Error ? err.message : "run-research failed";
      req.log?.error?.({ err }, "run-research error");
