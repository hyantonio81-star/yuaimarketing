...
function normalizeUrl(req: { url?: string; headers?: Record<string, string | string[] | undefined> }): string {
  ...
  try {
    ...
    else if (!p.startsWith("/")) p = p.startsWith("api") ? `/${p}` : `/api/${p}`;
  } catch {
    p = p.startsWith("/") ? p : `/api/${p}`;
  }

  // '/health', '/users' 처럼 '/api' 없이 전달되는 경우가 있어 라우트 매칭을 위해 보정합니다.
  if (p.startsWith("/") && !p.startsWith("/api/") && p !== "/api") {
    p = `/api${p}`;
  }
  ...
  if (url === "/health") {
    try {
      ...
      res.status(503).json({
        ...
        error: "Backend not built or failed to load",
      });
      return;
    }
  }
