...
| **Pillar 4** | Gov Tender | 입찰 공고·규격·참가자격을 **입찰 시장 국가** 기준. **현재 수동 모드**(접어둠). 활성화 시 `GOV_TENDER_ENABLED=true` 설정. |

---

**Pillar 4 수동 모드**: 정부 입찰 기능은 기본적으로 접어두었습니다. 사용하려면 서버(또는 Vercel 환경변수)에 `GOV_TENDER_ENABLED=true`를 설정하세요. API는 `GET /api/gov/status`로 `mode: "active" | "manual"`을 반환합니다. 자세한 내용은 [IMPROVEMENT_PLAN_보강안.md](./IMPROVEMENT_PLAN_보강안.md) Part 1 참고.
...
- **수동 모드**: 기본적으로 `GOV_TENDER_ENABLED !== 'true'` 이면 모든 Gov API가 503 + manual 메시지를 반환합니다. 활성화 시 위 로직이 동작합니다.
