...
- **로그인**: Supabase Auth 사용. `VITE_SUPABASE_*` 설정 시 `/login` 없이는 대시 접근 불가(ProtectedRoute). `VITE_REQUIRE_LOGIN=true`면 Supabase 미설정이어도 로그인 화면 먼저 노출(개발용 "로그인 없이 진행"은 개발 빌드에서만 노출, 프로덕션에서는 비노출).
...
- **B2B/B2C 인증·레이트 리밋**: `/api/b2b`, `/api/b2c` 전체에 로그인 필수(`requireUser`) + `checkRateLimitApi` 적용. IP당 1분 120회 초과 시 429 반환.
- **Shorts API 인증**: `/api/shorts/*` 전 구간 로그인 필수. Google OAuth callback(`GET /youtube/callback`)만 예외(리다이렉트라 JWT 없음).
...
- **Tienda Admin 로그인**: `POST /api/landing/admin/login`에 IP당 1분 5회 rate limit(`checkRateLimitLogin`). 비밀번호 비교는 SHA-256 해시 후 `crypto.timingSafeEqual`로 타이밍 공격 완화.
...
| 항목 | 권장 내용 |
|------|-----------|
| **로그인 우선** | Vercel에 `VITE_REQUIRE_LOGIN=true` 설정 시 접속 시 항상 로그인 화면 먼저 노출. |
...
| **CORS** | 프로덕션에서는 `ALLOWED_ORIGINS`에 허용 도메인(예: `https://yuaimarketing.vercel.app`)을 쉼표로 설정해 제한. 미설정 시 `origin: true`. |
| **세션** | Supabase가 세션·갱신 처리. 필요 시 JWT 만료·리프레시 설정은 Supabase Auth 설정에서 조정. |

---

## 3. 보안 권장 체크리스트 (배포·운영 시)

배포 후 또는 정기 점검 시 아래를 확인하세요.

| # | 항목 | 확인 방법 |
|---|------|-----------|
| 1 | **첫 접속 시 로그인** | `VITE_REQUIRE_LOGIN=true` 설정(프론트 `.env` 또는 Vercel 환경 변수) 후 재배포. 시크릿 창에서 사이트 접속 시 로그인 화면으로 리다이렉트되는지 확인. |
| 2 | **CORS 제한** | 프로덕션 백엔드에 `ALLOWED_ORIGINS=https://your-domain.vercel.app`(쉼표로 복수 도메인 가능) 설정. 미설정 시 모든 오리진 허용. |
| 3 | **Supabase 비밀번호 정책** | Supabase Dashboard → Authentication → Settings에서 최소 길이·복잡도(대소문자·숫자·특수문자) 설정 권장. |
| 4 | **RLS** | Supabase 테이블별 Row Level Security 활성화 및 정책으로 접근 제한. 새 테이블 추가 시 RLS 정책 작성. |
| 5 | **Admin bootstrap 비활성화** | 첫 관리자 생성 후 Vercel/서버 환경 변수에서 `ADMIN_BOOTSTRAP_ENABLED=false` 설정(또는 변수 제거). |
| 6 | **2FA (선택)** | Supabase Dashboard → Authentication → MFA에서 관리자 계정용 이중 인증 활성화. |
| 7 | **환경 변수** | `.env`는 Git에 포함하지 않음. Vercel·Supabase 등은 대시보드에서만 설정. `service_role` 키는 백엔드 전용, 프론트 노출 금지. |
...
- **CORS 제한**: `ALLOWED_ORIGINS` 설정으로 이미 허용 도메인만 명시 가능. 위 §3 체크리스트 참고.
