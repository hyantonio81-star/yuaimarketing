...
function getError(t, e) {
  return getApiErrorMessage(e, t("b2cCommerce.errSkuRequired"));
}

/** @param {{ sku: string; title?: string; price?: number } | null} [initialProduct] pre-fill from affiliate simulation */
export default function PromotionPlanSection({ initialProduct = null }) {
  const { t } = useLanguage();
  ...
  useEffect(() => {
    if (!initialProduct?.sku) return;
    setPromoSku(initialProduct.sku);
    if (initialProduct.title) setPromoName(initialProduct.title);
    if (initialProduct.price != null) setPromoBasePrice(String(initialProduct.price));
  }, [initialProduct?.sku, initialProduct?.title, initialProduct?.price]);
  ...
  const badgeKey = result?.execution_plan?.ai_messaging
    ? "b2cCommerce.implementationLevelSimulationAndAiCopy"
    : "b2cCommerce.implementationLevelSimulation";

  return (
    <SectionCard title={t("b2cCommerce.promotionTitle")} className="mb-6">
      <p className="text-sm text-muted-foreground mb-1">{t("b2cCommerce.promotionDesc")}</p>
      <p className="text-xs text-muted-foreground mb-4">({t(badgeKey)})</p>
