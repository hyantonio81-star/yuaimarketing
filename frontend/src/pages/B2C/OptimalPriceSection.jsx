...
/** @param {{ sku: string; price?: number; suggested_cost?: number } | null} [initialProduct] pre-fill from affiliate simulation */
export default function OptimalPriceSection({ connectedChannels = [], initialProduct = null }) {
  ...
  useEffect(() => {
    ...
    setPriceSku(initialProduct.sku);
    if (initialProduct.suggested_cost != null) setPriceCost(String(initialProduct.suggested_cost));
    if (initialProduct.price != null) setPriceCurrent(String(initialProduct.price));
  }, [initialProduct?.sku, initialProduct?.suggested_cost, initialProduct?.price]);
  ...
  return (
    <SectionCard title={t("b2cCommerce.optimalPriceTitle")} className="mb-6">
      <p className="text-sm text-muted-foreground mb-1">{t("b2cCommerce.optimalPriceDesc")}</p>
