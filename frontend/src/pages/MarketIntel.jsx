...
import { MessageCircle, BarChart3, ListOrdered, AlertTriangle, ShoppingBag, ArrowRight, Save, FileSearch, ExternalLink, Download, FileText, Newspaper } from "lucide-react";
...
import { downloadCsv, downloadJson } from "../lib/exportUtils.js";
...
export default function MarketIntel() {
  ...
  const [researchNewsItems, setResearchNewsItems] = useState([]);
  ...
  const [newsSummaryCountry, setNewsSummaryCountry] = useState("");
  const [newsInterestCategories, setNewsInterestCategories] = useState([]);
  const [error, setError] = useState(null);
  const [reportGenMessage, setReportGenMessage] = useState(null);
  const [reportGenLoading, setReportGenLoading] = useState(false);
  const [lastReportDownloadUrl, setLastReportDownloadUrl] = useState(null);

  const NEWS_INTEREST_OPTIONS = [
    { value: "b2b", tKey: "b2b" },
    { value: "b2c", tKey: "b2c" },
    { value: "both", tKey: "both" },
  ];
  ...
  useEffect(() => {
    if (currentCountryCode && !newsSummaryCountry) setNewsSummaryCountry(currentCountryCode);
  }, [currentCountryCode, newsSummaryCountry]);

  const filteredNewsItems = Array.isArray(newsItems) ? newsItems : [];
  ...
  const handleViewSegmentedResults = async () => {
    ...
    if (!(granularRequest.country_code ?? "").trim()) {
      setError(t("marketIntel.selectCountryForResearch"));
      return;
    }
    setError(null);
    setResultsLoading(true);
    setSegmentedResult(null);
    setResearchNewsItems([]);
    try {
      const country = getCurrentCountryCode() ?? "ALL";
      const res = await marketIntelApi.runResearch({
        orgId: "default",
        country,
        lang: reportOptions.language || uiLanguage || "ko",
        country_code: granularRequest.country_code.trim(),
        item: granularRequest.item || undefined,
        hs_code: granularRequest.hs_code || undefined,
        research_types: granularRequest.research_types,
      });
      setSegmentedResult(res?.segmented_result ?? null);
      setResearchNewsItems(Array.isArray(res?.news_items) ? res.news_items : []);
    } catch (e) {
      const msg = e?.response?.data?.error ?? e?.message;
      const isNetworkError = !e?.response || (typeof msg === "string" && msg.toLowerCase().includes("network"));
      setError(
        isNetworkError
          ? t("marketIntel.loadFailedNetwork")
          : typeof msg === "string"
            ? msg
            : t("marketIntel.loadFailed")
      );
    }
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      ...
      if (job?.download_url) setLastReportDownloadUrl(job.download_url);
  ...
  const exportSegmentedCompanies = () => {
    if (!segmentedResult?.related_companies?.length) return;
    const rows = segmentedResult.related_companies.map((rc) => ({
      company_name: rc.company_name ?? "",
      country_code: rc.country_code ?? "",
      products_or_hs: rc.products_or_hs ?? "",
      contact_email: rc.contact?.email ?? "",
      contact_phone: rc.contact?.phone ?? "",
      contact_source: rc.contact?.source ?? "",
      reason: rc.reason ?? "",
    }));
    downloadCsv(rows, "market-intel-segmented-companies.csv");
  };
  const exportSegmentedDominance = () => {
    if (!segmentedResult?.market_dominance?.length) return;
    const rows = [];
    segmentedResult.market_dominance.forEach((md) => {
      (md.top_players ?? []).forEach((p) => {
        rows.push({
          research_type: md.research_type_label ?? md.research_type ?? "",
          metric: md.metric ?? "",
          player_name: p.name ?? "",
          share_or_value: p.share_or_value ?? "",
          description: p.description ?? "",
        });
      });
    });
    if (rows.length) downloadCsv(rows, "market-intel-segmented-dominance.csv");
  };
  const exportFlowData = () => {
    ...
    downloadCsv(rows, "market-intel-flow-tools-services-outputs.csv");
  };
  const exportSources = () => {
    ...
    downloadCsv(rows, "market-intel-data-sources.csv");
  };
  const exportNews = () => {
    const rows = (filteredNewsItems ?? []).map((item) => ({
      title: item?.title ?? "",
      summary: item?.summary ?? "",
      source: item?.source ?? "",
      date: item?.date ?? "",
      url: item?.url ?? "",
      b2b_b2c: item?.b2b_b2c ?? "",
    }));
    downloadCsv(rows, "market-intel-news-summary.csv");
  };
  const exportReportSettings = () => {
    const sectionLabels = (reportMeta?.sections ?? []).filter((s) => (reportOptions?.sections ?? []).includes(s?.id)).map((s) => s?.label ?? s?.id).join("; ");
    const payload = {
      format: reportOptions?.format,
      scope: reportOptions?.scope,
      schedule: reportOptions?.schedule,
      language: reportOptions?.language,
      report_tier: reportOptions?.report_tier,
      target_page_count: reportOptions?.target_page_count,
      sections_selected: reportOptions?.sections ?? [],
      sections_labels: sectionLabels,
    };
    downloadJson(payload, "market-intel-report-settings.json");
  };
  ...
  return (
    <div className="p-6 lg:p-8 min-h-[60vh] relative z-0" role="main" aria-label={t("nav.marketIntel")}>
          ...
          <div className="space-y-4 pt-2 border-t border-border">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              <div>
                ...
                {!(granularRequest.country_code ?? "").trim() && (
                  <p className="text-xs text-amber-600 dark:text-amber-400 mt-1">{t("marketIntel.selectCountryForResearch")}</p>
                )}
              </div>
              <div>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <button
                ...
                disabled={resultsLoading || !((granularRequest.research_types ?? []).length >= 1) || !(granularRequest.country_code ?? "").trim()}
              ...
              >
                ...
                {resultsLoading ? t("common.loading") : t("marketIntel.runResearch")}
              </button>
            </div>
            <div id="market-intel-segmented">
            {segmentedResult && (
              <div className="mt-6 pt-4 border-t border-border space-y-4">
                ...
                {(researchNewsItems?.length ?? 0) > 0 && (
                  <div className="mt-4 p-3 rounded-lg border border-border bg-muted/20">
                    <h4 className="text-sm font-semibold text-foreground mb-2">{t("marketIntel.researchNewsTitle")}</h4>
                    <div className="space-y-2">
                      {researchNewsItems.map((item, i) => {
                        const Wrapper = item?.url ? "a" : "div";
                        const wrapperProps = item?.url
                          ? { href: item.url, target: "_blank", rel: "noreferrer noopener", className: "block p-2 rounded-md border border-border bg-card hover:bg-muted/50" }
                          : { className: "p-2 rounded-md border border-border bg-card" };
                        return (
                          <Wrapper key={i} {...wrapperProps}>
                            <div className="flex items-center gap-2 flex-wrap">
                              <span className="font-medium text-foreground text-sm">{item?.title ?? ""}</span>
                              {item?.url && <ExternalLink className="w-3.5 h-3.5 text-primary shrink-0" aria-hidden />}
                            ...
                            <p className="text-xs text-muted-foreground">{t("marketIntel.contactSource")}: {item?.source ?? ""}</p>
                          </Wrapper>
                        );
                      })}
                    </div>
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  {(segmentedResult?.related_companies?.length ?? 0) > 0 && (
                    <button type="button" onClick={exportSegmentedCompanies} className="inline-flex items-center gap-2 rounded-md border border-border px-3 py-1.5 text-sm font-medium text-foreground hover:bg-muted">
                      <Download className="w-4 h-4" />
                      {t("marketIntel.exportCsv")} ({t("marketIntel.relatedCompanies")})
                    </button>
                  )}
                  {(segmentedResult?.market_dominance?.length ?? 0) > 0 && (
                    <button type="button" onClick={exportSegmentedDominance} className="inline-flex items-center gap-2 rounded-md border border-border px-3 py-1.5 text-sm font-medium text-foreground hover:bg-muted">
                      <Download className="w-4 h-4" />
                      {t("marketIntel.exportCsv")} ({t("marketIntel.marketDominance")})
                    </button>
                  )}
                </div>
              </div>
      ...
      <SectionCard title={t("marketIntel.flowTitle")} className="mb-6">
            ...
            <div className="flex-1 rounded-lg border border-border bg-muted/30 p-4">
              ...
              <div className="space-y-2">
                {(Array.isArray(results) ? results : []).map((r, idx) => (
                  <ResultBlock
                    key={r?.id ?? `result-${idx}`}
                    icon={r?.id === "needs" ? ListOrdered : r?.id === "complaints" ? AlertTriangle : r?.id === "segmented" ? BarChart3 : r?.id === "news_summary" ? Newspaper : r?.id === "report" ? FileText : ShoppingBag}
                    title={r?.title ?? ""}
                    sub={r?.period ?? ""}
        ...
        {!loading && (tools?.length > 0 || services?.length > 0 || results?.length > 0) && (
          <div className="mt-4 pt-4 border-t border-border">
            <button type="button" onClick={exportFlowData} className="inline-flex items-center gap-2 rounded-md border border-border px-3 py-1.5 text-sm font-medium text-foreground hover:bg-muted">
              <Download className="w-4 h-4" />
              {t("marketIntel.exportCsv")}
            </button>
          </div>
        )}
      </SectionCard>
      ...
      <SectionCard title={t("marketIntel.sourcesSection")} className="mb-6">
        ...
        {(sourcesList?.length ?? 0) > 0 && (
          <div className="mt-4 pt-4 border-t border-border">
            <button type="button" onClick={exportSources} className="inline-flex items-center gap-2 rounded-md border border-border px-3 py-1.5 text-sm font-medium text-foreground hover:bg-muted">
              <Download className="w-4 h-4" />
              {t("marketIntel.exportCsv")}
            </button>
          </div>
      ...
      <SectionCard title={t("marketIntel.newsSummaryTitle")} className="mb-6">
        <div id="market-intel-news">
        <p className="text-xs text-muted-foreground mb-3">{t("marketIntel.newsSummarySubtitle")}</p>
        <div className="flex flex-wrap items-end gap-4 mb-4">
          <div>
            <label className="block text-xs font-medium text-muted-foreground mb-1">{t("marketIntel.country")}</label>
            <select
              value={newsSummaryCountry || currentCountryCode || ""}
              onChange={(e) => setNewsSummaryCountry(e.target.value)}
              className="rounded-md border border-input bg-background px-3 py-2 text-foreground text-sm min-w-[140px]"
            >
              <option value="">—</option>
              {(countries ?? []).map((c) => (
                <option key={c?.country_code ?? ""} value={c?.country_code ?? ""}>{c?.name ?? ""} ({c?.country_code ?? ""})</option>
          ...
          <div>
            <label className="block text-xs font-medium text-muted-foreground mb-1">{t("marketIntel.interestCategories")}</label>
            <div className="flex flex-wrap gap-3">
              {NEWS_INTEREST_OPTIONS.map((opt) => (
                <label key={opt.value} className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={(newsInterestCategories ?? []).includes(opt.value)}
                    onChange={(e) => {
                      setNewsInterestCategories((prev) =>
                        e.target.checked ? [...(prev ?? []), opt.value] : (prev ?? []).filter((v) => v !== opt.value)
                      );
                    }}
                    className="rounded border-input"
                  />
                  <span className="text-sm text-foreground">{t(`marketIntel.${opt.tKey}`)}</span>
                </label>
              ))}
            </div>
          </div>
          <div>
            <button type="button" onClick={exportNews} disabled={filteredNewsItems.length === 0} className="inline-flex items-center gap-2 rounded-md border border-border px-3 py-1.5 text-sm font-medium text-foreground hover:bg-muted disabled:opacity-50 disabled:cursor-not-allowed">
              <Download className="w-4 h-4" />
              {t("marketIntel.exportCsv")}
            </button>
          </div>
        ...
        <div className="space-y-3">
          {filteredNewsItems.length === 0 && (
            <p className="text-sm text-muted-foreground py-4">{!effectiveNewsCountry ? t("marketIntel.selectCountryForNews") : t("marketIntel.noNewsMatch")}</p>
          )}
          {filteredNewsItems.map((item, i) => {
            const Wrapper = item?.url ? "a" : "div";
            const wrapperProps = item?.url
              ? { href: item.url, target: "_blank", rel: "noreferrer noopener", className: "block p-3 rounded-lg border border-border bg-muted/20 hover:bg-muted/40 hover:border-primary/30 transition-colors" }
              : { className: "p-3 rounded-lg border border-border bg-muted/20" };
            return (
              <Wrapper key={i} {...wrapperProps}>
                <div className="flex items-center gap-2 flex-wrap mb-1">
                  <span className="font-medium text-foreground">{item?.title ?? ""}</span>
                  {item?.url && <ExternalLink className="w-4 h-4 text-primary shrink-0" aria-hidden />}
                  {item?.b2b_b2c && (
                    <span className="px-1.5 py-0.5 rounded text-xs bg-accent/20 text-accent">
      ...
      <SectionCard title={t("marketIntel.reportSettingsTitle")} className="mb-6">
        <div id="market-intel-report" className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              >
                <Download className="w-4 h-4" />
                {t("marketIntel.exportJson")}
              </button>
            </div>
            {reportGenMessage && (
              <p className="text-sm text-muted-foreground mt-2 p-2 rounded bg-muted/50" role="status">
                {reportGenMessage}
                {lastReportDownloadUrl && (
                  <>
                    {" "}
                    <a
                      href={lastReportDownloadUrl.startsWith("http") ? lastReportDownloadUrl : `${(import.meta.env.VITE_API_URL || (typeof window !== "undefined" ? window.location.origin : "")).replace(/\/$/, "")}${lastReportDownloadUrl}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="font-medium text-primary underline"
                    >
                      {t("marketIntel.downloadReport")}
                    </a>
                  </>
                )}
              </p>
...
function ResultBlock({ icon: Icon, title, sub, color, actionPath, outputType }) {
  ...
  const scrollToSection = () => {
    ...
    const el = typeof document !== "undefined" ? document.querySelector(actionPath) : null;
    if (el) el.scrollIntoView({ behavior: "smooth", block: "start" });
    else if (actionPath.startsWith("#")) window.location.hash = actionPath;
  };
  return (
