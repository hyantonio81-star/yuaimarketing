...
2. **FFmpeg가 설치된 PC·VPS**에서 같은 리포지토리를 클론한 뒤, 아래 값을 **환경 변수** 또는 **`backend/.env`**에 넣습니다.  
   워커 스크립트는 시작 시 `backend/.env`를 읽어 비어 있는 키만 채웁니다(이미 셸에 export 된 값이 있으면 그대로 우선).

   | 변수 | 예시 |
   |------|------|
   | `SHORTS_API_BASE` | `https://your-app.vercel.app/api/shorts` (끝 슬래시 없음) |
   | `SHORTS_WORKER_SECRET` | Vercel과 **동일**한 값 |
   | `FFMPEG_PATH` | (선택) Windows BtbN/Gyan 등 `bin\ffmpeg.exe` 전체 경로 — `backend/.env`에만 있어도 됨 |
   | `SUPABASE_URL` | 백엔드와 동일 (`backend/.env`에 이미 있으면 추가 설정 불필요) |
   | `SUPABASE_SERVICE_ROLE_KEY` | 백엔드와 동일 |

   조립 CLI(`runManifestAssembly`)는 `backend` 폴더에서 실행되며 `dotenv`로 **`backend/.env`**를 다시 로드하므로, **`FFMPEG_PATH`는 백엔드 `.env` 한곳에만** 두어도 조립에 반영됩니다.
